...
class TomatoPublisher(Node):
    def __init__(self):
        ...
        # YOLOv5 모델 로드
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='/home/shj/tomato/tomato_temp/src/tomato_detection/tomato_detection/best.pt')
        ...
        # 카메라 캘리브레이션 정보 로드
        calibration_file = '/home/shj/tomato/tomato_temp/src/tomato_detection/tomato_detection/camera_calibration.xml'
    ...
    def publish_tomato_coordinates(self):
        ...
        for detection in results.xyxy[0]:
            if detection[4] >= 0.8 and detection[5] in [0, 5]:  # Confidence score가 0.8 이상이고 "tomato" 또는 "upripe" 클래스 라벨을 나타내는 인덱스
                x, y, w, h = detection[:4]
